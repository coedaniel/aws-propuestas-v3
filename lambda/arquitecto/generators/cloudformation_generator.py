...
def generate_cloudformation_template(project_info: Dict[str, Any]) -> str:
    ...
    template = {
        ...
        'Parameters': get_parameters(project_type),
        'Resources': get_resources(project_type, project_name),
        'Outputs': get_outputs(project_type, project_name)
    }
    
    return yaml.dump(template, default_flow_style=False, allow_unicode=True)

def get_parameters(project_type: str) -> Dict[str, Any]:
    """Get CloudFormation parameters based on project type"""
    base_params = {
        'Environment': {
            'Type': 'String',
            'Default': 'prod',
...
def get_resources(project_type: str, project_name: str) -> Dict[str, Any]:
    """Get CloudFormation resources based on project type"""
    resources = {}
    
    # Base VPC resources
    resources.update(get_vpc_resources(project_name))
    
    # Add specific resources based on project type
    if 'web' in project_type.lower() or 'app' in project_type.lower():
        resources.update(get_web_app_resources(project_name))
    
    if 'data' in project_type.lower() or 'analitica' in project_type.lower():
        resources.update(get_analytics_resources(project_name))
    
    if 'iot' in project_type.lower():
        resources.update(get_iot_resources(project_name))
    
    if 'migracion' in project_type.lower():
        resources.update(get_migration_resources(project_name))
    
    # Add common security and monitoring resources
    resources.update(get_security_resources(project_name))
    resources.update(get_monitoring_resources(project_name))
