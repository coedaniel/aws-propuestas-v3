"""
HTTP wrapper for diagram MCP server with path prefix support
"""
...
app = FastAPI(
    title=f"diagram MCP Server",
    root_path="/diagram",
    docs_url="/diagram/docs",
    redoc_url="/diagram/redoc"
)
...
async def root():
    """Root endpoint info"""
    return {
        "service": "diagram MCP Server",
        "status": "running",
        "prefix": "/diagram",
        "endpoints": ["/diagram/health", "/diagram/tools", "/diagram/call-tool"]
    }

@app.get("/health")
@app.get("/diagram/health")
...
async def execute_mcp_tool(tool_name: str, arguments: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        cmd = mcp_commands.get("diagram", ["echo", "unknown"])
