"""
HTTP wrapper for awsdocs MCP server with path prefix support
"""
...
app = FastAPI(
    title=f"awsdocs MCP Server",
    root_path="/awsdocs",
    docs_url="/awsdocs/docs",
    redoc_url="/awsdocs/redoc"
)
...
async def root():
    """Root endpoint info"""
    return {
        "service": "awsdocs MCP Server",
        "status": "running",
        "prefix": "/awsdocs",
        "endpoints": ["/awsdocs/health", "/awsdocs/tools", "/awsdocs/call-tool"]
    }

@app.get("/health")
@app.get("/awsdocs/health")
...
async def execute_mcp_tool(tool_name: str, arguments: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        cmd = mcp_commands.get("awsdocs", ["echo", "unknown"])
